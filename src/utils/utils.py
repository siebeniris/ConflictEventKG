...
def get_params(cwd):
    ...
    return: fields.
    """
    config_dir = os.path.join(cwd, "src", 'crawler', 'config', 'fields_expansions')
...
if __name__ == '__main__':
    import re
    file = "../data/entities_wiki_dedup.csv"
    df = pd.read_csv(file)
    entities = df["entity_name"].str.replace(" and ", " ").tolist()
    entities = ["".join(re.split("\(|\)|\[|\]", x)[::2]).lower() for x in entities]
    chunks_ = list(chunks(entities, 30))
    with open("../data/keywords_chunks.json", "w") as file:
        json.dump(chunks_, file)
